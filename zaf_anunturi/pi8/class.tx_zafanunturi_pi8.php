 ...
 */
class tx_zafanunturi_pi8 extends tslib_pibase {
	public $prefixId      = 'tx_zafanunturi_pi8';		// Same as class name
	public $scriptRelPath = 'pi8/class.tx_zafanunturi_pi8.php';	// Path to this script relative to the extension dir.
	...
	public function main($content, array $conf) {
		...
		$user_ID = $GLOBALS['TSFE']->fe_user->user; 
		$this->userID = $user_ID['uid'];
		
		//delete anunt
		if($this->piVars['delete_id'] && $this->piVars['delete_id'] > 0) {
			$this->delete($this->piVars['delete_id']);
		}
...
//debug($this->pi_getFFvalue($this->cObj->data['pi_flexform'],'metoda_afisare'),'flex method');		
	...
	function delete($id) {
		...
		$GLOBALS['TYPO3_DB']->exec_UPDATEquery('tx_zafanunturi_anunuri', "uid='".intval($id)."' AND user_id='".intval($this->userID)."'", $updateArray);
		//debug($GLOBALS['TYPO3_DB']->UPDATEquery('tx_zafanunturi_anunuri', "uid='".intval($id)."' AND user_id='".intval($this->userID)."'", $updateArray),'query');
		setcookie("ZafSucessDelete", 'removed', time()+80);
		header('Location:'.$this->pi_getPageLink($GLOBALS['TSFE']->id,'',array())); exit();		
	}
	...
	function init() {
		$this->conf['templateFile'] =  $this->conf['templateFile'] ? $this->conf['templateFile'] : 'EXT:' . $this->extKey . '/res/template_pi8.html';
...
//debug($GLOBALS['TSFE']->fe_user->user,'user');
...
//debug($where,'where');		
		$rows = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows($selectedRows, 'tx_zafanunturi_anunuri', $where, '', $sorting, $limit);
//debug($rows = $GLOBALS['TYPO3_DB']->SELECTquery($selectedRows, 'tx_zafanunturi_anunuri', $where, '', $sorting, $limit),'query');		
...
//debug($row,'row');			
			...
			$linkConfEdit = array(
						'parameter' => $this->conf['editPID'],
						'additionalParams' => '&tx_zafanunturi_pi1[edit_id]='.$row['uid']
			);
			
			$linkConfDelete = array(
						'parameter' => $GLOBALS['TSFE']->id,
						'additionalParams' => '&tx_zafanunturi_pi8[delete_id]='.$row['uid'],
						'ATagParams'	=> 'data-confirm="Esti sigur ca doresti sa stergi acest anunt?"'
			);
			...
			$accum .= $this->cObj->substituteMarkerArray($this->subTemplate, 
																array('###DATA###'	 		=> $data,
																      ...
																      '###ID###'			=> $row['uid'],
																	  '###EDIT###'			=> $this->cObj->typolink('Editeaza',$linkConfEdit),	
																	  '###DELETE###'		=> $this->cObj->typolink('Sterge',$linkConfDelete),
														 	  		 )
		 	   												   );
		 	$counter++;	   												  
		}
...
if (defined('TYPO3_MODE') && isset($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/zaf_anunturi/pi8/class.tx_zafanunturi_pi8.php'])) {
	include_once($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/zaf_anunturi/pi8/class.tx_zafanunturi_pi8.php']);
}
