...
class image_resizer
{
	...
	function image_resizer($source=null)
	{
		if ($source)
	...
	function setSource($source)
	{
		...
		$this->imgProp	= getimagesize($source);

		if (!count($this->imgProp))
		{
			$this->error	=	'File type not supported! (Supported: jpg,gif,png)';
			return false;
		}

		switch($this->imgProp[2]) {
			case 2:
			...
			case 1:
				...
				break;
			default:
				$this->error	=	'Not supported file type! (Supported file types: jpg,gif,png)';
				return false;
		}
	...
	function resize($path,$size, $filename=null, $quality=100) {

		$path	.=	(substr($path,-1)=='/')?'':'/';

		$width 	= $this->imgProp[0];
		$height = $this->imgProp[1];
		...
		if ( ($imgRes = imagecreatetruecolor($newwidth, $newheight)) === FALSE )
		{
			$this->error = 'Error: Can\'t create a new true color image!';
			return false;
		}
/*
		echo '<br />source='.$this->source.'<br />';
		echo 'img res='.$imgRes.'<br />';
		echo 'new width='.$newwidth.'<br />';
		echo 'new height='.$newheight.'<br />';
		echo 'width='.$width.'<br />';
		echo 'height='.$height.'<br />';
*/		
		if (imagecopyresampled($imgRes, $this->source, 0, 0, 0, 0, $newwidth, $newheight, $width, $height) === FALSE )
		{
			$this->error = 'Error: Can\'t copy or resize the image! (Check file permissions)';
			return false;
		}

		if ( !imagejpeg($imgRes, $path.$filename, $quality) )
		{
			$this->error = 'Error: Can\'t store the image! (Check file permissions)';
			return false;
		}
...
function resizeAndCrop($path, $size, $filename=null, $quality=100) {
		...
		$newWidth  = $width;
		$newHeight = round(($width * 66) / 100);
		
		$filename	=	($filename)?$filename:$this->generateFileName();

		
//start crop and resize
		if ( ($imgRes = imagecreatetruecolor(100, 66)) === FALSE )
		{
			$this->error = 'Error: Can\'t create a new true color image!';
			return false;
		}

		if (imagecopyresampled($imgRes, $this->source, 0, 0, 0, 0, 100, 66, $newWidth, $newHeight) === FALSE )
		{
			$this->error = 'Error: Can\'t copy or resize the image! (Check file permissions)';
			return false;
		}
